...
for train_cv, test_cv in kf.split(train_usr):
    ...
    I_cv_sim = np.zeros((test_cv_pl.shape[0],num_track))    # Matrix for user similariity scores
        
    train_cv_norm = 1/train_cv_usr.sum(axis=0).A.ravel()
    
    # Running for different hyperparameters
    for alph in alph_arr:
        train_cv_usr_col = train_cv_usr.dot(diags(np.power(train_cv_norm,alph)))
        train_cv_usr_row = train_cv_usr.dot(diags(np.power(train_cv_norm,1-alph)))
        I_cv_score = train_cv_usr_row.transpose().dot(train_cv_usr_col)
...
#        train_cv_usr_c = train_cv_usr.tocsc()
        ...
        for q in q_arr:
            I_cv_score_mat = I_cv_score.power(q)
            U_simI = np.zeros((test_cv_size,num_track))
            
            srt1 = timeit.default_timer()
            print('Alpha = ',alph, ', q = ',q)
            for it in tqdm(range(test_cv_size)):
                test_cv_pl_it = test_cv_pl[it,:]
                test_col_ind = test_cv_pl_it.nonzero()[1]     #Find track index already in the test_pl
                
                Iu = I_cv_score_mat[test_col_ind,:]      # Scores for only those songs
                U_simI[it,:] = Iu.sum(axis=0)
                if(it%1000==0):
                    print('abab')
                    stp1 = timeit.default_timer()
                    print(stp1-srt1)
...
plt.hist(rprec_pl,density=True)
plt.show()
